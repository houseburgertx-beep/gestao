...
function TransferModal({ accounts, tenantId, onClose, onSaved }: { accounts: RecordData[]; tenantId: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="mg-modal-shade"><div className="mg-modal" role="dialog" aria-modal="true"><header><h2>Transferência entre bancos</h2><button onClick={onClose}>×</button></header><form className="mg-form" onSubmit={async (event) => {event.preventDefault(); if(!user)return; setBusy(true);setError("");try {const form=new FormData(event.currentTarget);const from=String(form.get("from")||"");const to=String(form.get("to")||"");if(!from||!to||from===to)throw new Error("Selecione contas diferentes.");const origin=accounts.find(a=>a.id===from);if(!origin)throw new Error("Conta de origem inválida.");const now=new Date().toISOString();const row:RecordData={id:safeUUID(),kind:"bankTransfers",tenantId,unitId:origin.unitId,version:0,createdAt:now,updatedAt:now,createdBy:user.uid,updatedBy:user.uid,fromBankId:from,toBankId:to,date:String(form.get("date")),amount:Math.round(Number(form.get("amount"))*100),notes:String(form.get("notes")||"")};await saveManagement(row,data);onSaved();}catch(e){setError(e instanceof Error?e.message:"Não foi possível transferir.");}finally{setBusy(false);}}}>
